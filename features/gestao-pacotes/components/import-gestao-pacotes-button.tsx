...
type ValidationResult = {
  ...
  sheets: Array<{ name: string; acceptedRows: number; ignoredRows: number; sheetType: string; reason?: string }>;
  message: string;
};

export function ImportGestaoPacotesButton() {
  ...
  async function validate(formData: FormData) {
    ...
    setResult(null);
    try {
      const response = await fetch("/api/gestao-pacotes/validate", {
        method: "POST",
        body: formData,
      });
              ...
              {result ? (
                <div className="validation-result">
                  ...
                  <div>
                    ...
                    <span>Pacotes: {result.uniquePackages.toLocaleString("pt-BR")}</span>
                    <span>Eventos: {result.events.toLocaleString("pt-BR")}</span>
