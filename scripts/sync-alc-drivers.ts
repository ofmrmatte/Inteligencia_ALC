...
try {
  const [drivers, prefatura, pnr, risk] = await Promise.all([
  ...
  if (apply) {
    ...
    for (let index = 0; index < rows.length; index += 500) {
      ...
      await client.query(`
        insert into public.alc_drivers(driver_code, full_name, base_key, sigla, operational_status, last_operational_seen_at, source_updated_at, source_payload, updated_at)
        select
          driver_code,
          full_name,
          nullif(base_key, ''),
  ...
  console.log(JSON.stringify({ dryRun, apply, activityWindowDays: activityWindowDays(), activeCutoffDate: cutoffDate(), stats }, null, 2));
} catch (error) {
