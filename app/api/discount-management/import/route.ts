...
export async function POST(request: Request) {
  ...
  try {
    ...
    for (const row of parsed.rows) {
      const existingResult = await admin.from("discount_cases").select("id,direction,discount_month,source_period").eq("shipment_id", row.shipmentId).maybeSingle();
      ...
      if (existingResult.data) {
        ...
        const insert = await admin.from("discount_cases").insert({ ...common, shipment_id: row.shipmentId, created_by: profile.id }).select("id").single();
