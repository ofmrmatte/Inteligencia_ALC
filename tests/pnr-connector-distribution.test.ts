...
describe("distribuição do Conector PNR", () => {
  ...
  it("separa ausência da extensão, aba e sessão", () => {
    ...
    expect(connectorStateFromHandshake({ installed: true, version: "1.1.12", mlTabAvailable: false, sessionAvailable: false })).toBe("ml-missing");
    expect(connectorStateFromHandshake({ installed: true, version: "1.1.12", mlTabAvailable: true, sessionAvailable: false })).toBe("expired");
    expect(connectorStateFromHandshake({ installed: true, version: "1.1.12", mlTabAvailable: true, sessionAvailable: false, sessionError: "INVALID_RESPONSE" })).toBe("error");
    ...
    expect(connectorStateFromHandshake({ installed: true, version: "1.1.12", mlTabAvailable: true, sessionAvailable: true })).toBe("connected");
  });
  ...
  it("bloqueia versões anteriores ao mínimo e avisa quando há uma versão mais recente", () => {
    ...
    expect(LATEST_CONNECTOR_VERSION).toBe("1.1.12");
    expect(MINIMUM_SUPPORTED_CONNECTOR_VERSION).toBe("1.1.12");
  });
