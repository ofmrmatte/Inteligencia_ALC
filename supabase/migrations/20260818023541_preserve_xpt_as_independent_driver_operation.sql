...
declare
  ...
  resolved_sigla text;
begin
  -- XPT has precedence over SVC. It is an independent operational unit.
  ...
  -- Exact SVC + base from the master SVC directory.
  if exists (
  ...
  -- Repair legacy rows by unique SVC base only when no XPT was supplied.
...
-- XPT codes cannot simultaneously exist as SVC identifiers.
