alter table public.operational_bases add column if not exists id text generated always as (base_key) stored;
create unique index if not exists operational_bases_id_uidx on public.operational_bases (id);
      ...
      and (
        ...
        or (
          ...
          and (
            ...
            or (coalesce(record_sigla,'') <> '' and record_sigla = any(coalesce(p.sigla_scope,'{}'::text[])))
          )
        )
      )
  ), false)
...
returns boolean language sql stable security definer set search_path = public as $$
  with current_profile as (
    ...
    union all select p.base_key,p.sigla from public.prefatura_records p where p.batch_id = target_batch_id
    union all select p.base_key,p.sigla from public.pnr_records p where p.batch_id = target_batch_id
    union all select r.base_key,r.sigla from public.risk_lm_records r where r.batch_id = target_batch_id
  )
  select coalesce(exists (
...
returns boolean language sql stable security definer set search_path = public as $$
  ...
  select coalesce(exists (
    ...
    where cp.active = true and (
      cp.role in ('director','developer','loss_supervisor','loss_admin','super_admin')
      or (
        cp.role in ('coordinator','supervisor') and (
          ...
          or exists (select 1 from public.prefatura_records p where coalesce(target_driver_id,'') <> '' and p.driver_id = target_driver_id and ((coalesce(p.base_key,'') <> '' and p.base_key = any(cp.allowed_bases)) or (coalesce(p.sigla,'') <> '' and p.sigla = any(cp.allowed_siglas))))
          or exists (select 1 from public.pnr_records p where coalesce(target_driver_id,'') <> '' and p.driver_id = target_driver_id and ((coalesce(p.base_key,'') <> '' and p.base_key = any(cp.allowed_bases)) or (coalesce(p.sigla,'') <> '' and p.sigla = any(cp.allowed_siglas))))
          or exists (select 1 from public.risk_lm_records r where coalesce(target_driver_id,'') <> '' and r.driver_id = target_driver_id and ((coalesce(r.base_key,'') <> '' and r.base_key = any(cp.allowed_bases)) or (coalesce(r.sigla,'') <> '' and r.sigla = any(cp.allowed_siglas))))
          or exists (select 1 from public.prefatura_records p where coalesce(target_name,'') <> '' and app_private.normalize_scope_text(p.driver_name)=app_private.normalize_scope_text(target_name) and ((coalesce(p.base_key,'') <> '' and p.base_key = any(cp.allowed_bases)) or (coalesce(p.sigla,'') <> '' and p.sigla = any(cp.allowed_siglas))))
        )
      )
    )
  ), false)
...
update public.profiles set global_access = true, updated_at = now() where role::text = 'loss_admin';
...
create policy "bases scoped read" on public.operational_bases for select to authenticated using (app_private.can_manage_imports() or app_private.can_manage_driver_base(base_key));

drop policy if exists "drivers scoped read" on public.alc_drivers;
create policy "drivers scoped read" on public.alc_drivers for select to authenticated using (app_private.can_manage_imports() or app_private.can_manage_driver_base(base_key) or auth_user_id = auth.uid());
...
create policy "alc imports insert" on storage.objects for insert to authenticated with check (bucket_id = 'alc-imports' and app_private.can_manage_imports());
create policy "alc imports read" on storage.objects for select to authenticated using (bucket_id = 'alc-imports' and app_private.can_manage_imports());
create policy "alc imports update" on storage.objects for update to authenticated using (bucket_id = 'alc-imports' and app_private.can_manage_imports()) with check (bucket_id = 'alc-imports' and app_private.can_manage_imports());
create policy "alc imports delete" on storage.objects for delete to authenticated using (bucket_id = 'alc-imports' and app_private.can_manage_imports());
