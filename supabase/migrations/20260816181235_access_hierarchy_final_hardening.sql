-- Final hardening for the ALC access hierarchy.
-- Full-panel access is role-based; legacy global_access no longer elevates scoped roles.
...
as $$
  select
    ...
    or exists (
      ...
      where p.id = auth.uid()
        and p.active = true
        and p.role::text = 'administration_supervisor'
    )
    or exists (
      select 1
      from public.profiles p
...
-- Remove legacy profile policies that let a normal ADM read/update unrelated profiles
-- or let any user mutate their own authorization fields.
...
create policy "profiles read own or full manager"
on public.profiles
for select
to authenticated
using (id = auth.uid() or app_private.has_global_internal_access());

create policy "profiles full manager write"
on public.profiles
for all
to authenticated
...
-- Normalize legacy flags/scopes so database state matches the new hierarchy.
update public.profiles
set global_access = case
  when role::text in ('director', 'developer', 'loss_supervisor', 'super_admin') then true
  else false
end,
updated_at = now();
...
set module_scope = array['visao-geral','gestao-pnr','pre-faturamento','risco-lm','motoristas','perfil']::text[],
    ...
    updated_at = now()
where role::text in ('coordinator','supervisor');
...
set module_scope = array['gestao-motoristas']::text[],
    ...
    updated_at = now()
where role::text = 'administration_supervisor';
