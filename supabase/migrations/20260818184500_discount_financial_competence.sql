alter table public.discount_cases
  add column if not exists discount_month text,
...
set discount_month = case
  when source_period ~ '^0?[12]Q[0-9]{2}[0-9]{4}$' then
    substr(source_period, length(source_period) - 3, 4) || '-' || substr(source_period, position('Q' in source_period) + 1, 2)
  when manual_date is not null then to_char(manual_date, 'YYYY-MM')
  else discount_month
end
where discount_month is null;

alter table public.discount_cases
  drop constraint if exists discount_cases_discount_month_check;

alter table public.discount_cases
  add constraint discount_cases_discount_month_check
  check (discount_month is null or discount_month ~ '^[0-9]{4}-(0[1-9]|1[0-2])$');

create index if not exists discount_cases_discount_month_idx on public.discount_cases(discount_month);
create index if not exists discount_cases_source_batch_idx on public.discount_cases(source_batch_id);
...
select
  ...
  coalesce(pf.month, pn.month, case when c.manual_date is not null then to_char(c.manual_date, 'YYYY-MM') else null end) as operational_month,
  coalesce(pf.fortnight, pn.fortnight) as operational_fortnight,
  coalesce(
    c.discount_month,
    case
      when c.source_period ~ '^0?[12]Q[0-9]{2}[0-9]{4}$' then
        substr(c.source_period, length(c.source_period) - 3, 4) || '-' || substr(c.source_period, position('Q' in c.source_period) + 1, 2)
      else null
    end,
    pf.month,
    pn.month,
    case when c.manual_date is not null then to_char(c.manual_date, 'YYYY-MM') else null end
  ) as month,
  ...
  case
    ...
    when c.source_kind = 'spreadsheet' then 'Planilha Gestão de Descontos'
    else 'Manual'
  end as origin,
  greatest(
