...
begin
  ...
  -- Pré-fatura: atua somente no lote atual da fonte e preserva o registro mais recente.
  select r.batch_id
    into v_batch_id
  ...
  -- PNR: o status/data mais recente da ocorrência atual permanece.
  select r.batch_id
    into v_batch_id
  ...
  -- Risco LM: mantém a ocorrência mais recente do lote atual.
  select r.batch_id
    into v_batch_id
...
comment on function public.merge_reconciliation_duplicates_admin(text, uuid) is 'Mescla duplicidades do lote atual por fonte, preservando a ocorrência mais recente e auditando as removidas.';
