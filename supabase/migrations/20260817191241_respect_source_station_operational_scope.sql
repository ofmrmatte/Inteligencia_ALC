...
begin
  ...
  if coalesce(raw_station,'') <> '' then
    ...
    -- A estação/SVC informada pela fonte tem precedência. Nunca trocamos uma
    -- estação conhecida por outra SVC apenas porque o motorista já atuou nela.
    new.sigla:=raw_station;
    new.base_key:=raw_station;
    return new;
  end if;

  -- Somente quando a fonte não informou estação/facility podemos inferir a
  -- unidade a partir de um histórico unívoco do motorista.
  if coalesce(new.driver_id,'')<>'' then
    with evidence as (
      select u.unit_key,u.base_key,u.sigla
      from public.alc_drivers d
...
-- Recalcula os registros existentes usando a própria estação/facility como
-- fonte primária. Isto desfaz qualquer inferência histórica que tenha trocado
-- uma SVC por outra diferente.
...
-- SVCs que possuem uma única base são resolvidas sem ambiguidade.
...
set sigla=u.sigla,base_key=u.base_key
from unique_sigla u
...
-- Para SVCs com várias bases, utiliza somente evidência do mesmo motorista
-- dentro da mesma SVC. Se houver mais de uma base possível, permanece pendente.
with safe_evidence as (
  ...
  from safe_evidence
  group by driver_id,app_private.normalize_scope_text(sigla)
  having count(distinct unit_key)=1
)
update public.pnr_records p
set sigla=e.sigla,base_key=e.base_key
...
-- Quando a PNR não traz estação, uma unidade só é inferida se o histórico
-- completo do motorista apontar para uma única unidade operacional.
with safe_evidence as (
  select d.driver_code driver_id,u.unit_key,u.sigla,u.base_key
  from public.alc_drivers d
  join public.operational_units u on u.active=true
...
where coalesce(p.origin_station,'')=''
  and p.driver_id=e.driver_id;
