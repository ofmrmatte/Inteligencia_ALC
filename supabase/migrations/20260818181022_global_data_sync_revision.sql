...
declare
  ...
  tracked_tables text[] := array[
    'admin_base_assignments','alc_drivers','dashboard_files','dashboard_settings','desvios_pnr_records','discount_cases','driver_dispute_messages','driver_disputes','driver_notifications','driver_payment_batches','driver_payment_document_versions','driver_payment_documents','driver_portal_base_access','driver_records','gestao_desvios_pacotes_faltantes','gestao_pacotes_records','hierarchy_scopes','import_batches','imported_files','operational_bases','operational_unit_supervisors','operational_units','operational_xpts','pnr_records','pre_fatura_records','prefatura_records','processed_dashboard_files','profiles','quality_issues','risk_lm_records'
  ];
begin
  foreach target_table in array tracked_tables loop
    if to_regclass(format('public.%I', target_table)) is not null then
      ...
      execute format('create trigger global_data_revision_trigger after insert or update or delete or truncate on public.%I for each statement execute function app_private.bump_global_data_revision()', target_table);
    end if;
  end loop;
end;
$$;
