...
as $function$
  ...
  unique_sigla_unit as (
    select min(u.unit_key) as unit_key, min(u.sigla) as sigla, min(u.base_key) as base_key
    from public.operational_units u, normalized_record r
    where u.active = true
      and app_private.normalize_scope_text(u.sigla) = r.sigla
...
as $function$
  select coalesce(
    ...
    or exists (
      ...
      where p.id = auth.uid() and p.active = true and p.role::text = 'loss_admin'
    )
    or exists (
      select 1 from public.hierarchy_scopes h
      where h.batch_id = target_batch_id and app_private.can_read_scope(h.sigla, h.base_key)
    )
    or exists (
      select 1 from public.prefatura_records p
      where p.batch_id = target_batch_id and app_private.can_read_scope(p.sigla, p.base_key)
    )
    or exists (
    ...
    or exists (
      ...
      where r.batch_id = target_batch_id and app_private.can_read_scope(r.sigla, r.base_key)
    ),
    false
  )
$function$;
...
with driver_candidates as (
  ...
  select driver_code, min(base_key) as base_key, min(sigla) as sigla
  from driver_candidates
  group by driver_code
  having count(distinct unit_key) = 1
)
update public.pnr_records p
set base_key = r.base_key, sigla = r.sigla
from resolved_driver r
where p.driver_id = r.driver_code
  and not exists (
...
update public.risk_lm_records rlm
set base_key = r.base_key, sigla = r.sigla
from resolved_driver r
where rlm.driver_id = r.driver_code
  and not exists (
    select 1 from public.operational_units u
    where u.active = true
      and app_private.normalize_scope_text(u.sigla) = app_private.normalize_scope_text(rlm.sigla)
      and app_private.normalize_scope_text(u.base_key) = app_private.normalize_scope_text(rlm.base_key)
  );

with unique_siglas as (
  select app_private.normalize_scope_text(sigla) as normalized_sigla, min(base_key) as base_key, min(sigla) as sigla
  from public.operational_units
  where active = true
  group by app_private.normalize_scope_text(sigla)
  having count(*) = 1
)
update public.pnr_records p
set base_key = u.base_key, sigla = u.sigla
from unique_siglas u
where app_private.normalize_scope_text(p.sigla) = u.normalized_sigla
  and not exists (
    select 1 from public.operational_units z
    where z.active = true
      and app_private.normalize_scope_text(z.sigla) = app_private.normalize_scope_text(p.sigla)
      and app_private.normalize_scope_text(z.base_key) = app_private.normalize_scope_text(p.base_key)
  );
...
set base_key = u.base_key, sigla = u.sigla
from unique_siglas u
where app_private.normalize_scope_text(r.sigla) = u.normalized_sigla
  and not exists (
    select 1 from public.operational_units z
    where z.active = true
      and app_private.normalize_scope_text(z.sigla) = app_private.normalize_scope_text(r.sigla)
      and app_private.normalize_scope_text(z.base_key) = app_private.normalize_scope_text(r.base_key)
  );
