...
declare
  ...
  resolved_sigla text;
begin
  -- Already an exact SVC + base from the master directory.
  if exists (
    select 1
  ...
  -- A base name that exists in only one SVC is enough to repair legacy
  -- rows where XPT was incorrectly stored in the SVC column.
  ...
  -- Legacy rows sometimes stored the SVC itself as BASE_KEY (SGO1-SGO1,
  -- SSP5-SSP5, etc.). Only resolve that fallback when the SVC has exactly
  -- one master base; ambiguous SVCs remain pending instead of being guessed.
  ...
  -- XPT is an independent dimension. Never leave an XPT code masquerading
  -- as SVC when no reliable base resolution exists.
  if exists (
...
-- Prefer the latest exact historical SVC + base evidence for each driver.
with evidence as (
...
-- Run every still non-canonical driver through the conservative trigger so
-- unique base/SVC fallbacks are repaired and unresolved XPT codes are cleared.
update public.alc_drivers d
