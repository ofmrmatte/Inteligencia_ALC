...
begin
  ...
  if target_enabled then
    ...
    set portal_eligible = true,
        updated_at = now()
    from public.operational_bases b
    where d.base_key = b.base_key
      and upper(coalesce(nullif(b.sigla, ''), b.base_key)) = normalized_base_key
      and d.portal_status not in ('blocked', 'inactive')
      and d.portal_eligible is distinct from true;
    ...
    where d.base_key = b.base_key
      and upper(coalesce(nullif(b.sigla, ''), b.base_key)) = normalized_base_key
      and d.portal_eligible is distinct from false;
    get diagnostics drivers_updated = row_count;

    update public.driver_portal_sessions s
    set revoked_at = now()
    from public.alc_drivers d
    join public.operational_bases b on b.base_key = d.base_key
