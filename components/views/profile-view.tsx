...
export function ProfileView({ profile }: { profile: AuthProfile }) {
  ...
  return (
    <div className="view-stack">
      <PageIntro description="O perfil exibido vem da autenticação Supabase e define quais bases, siglas e ações ficam disponíveis no painel." chips={[ROLE_LABELS[profile.role], profile.email]} />
      <div className="kpi-grid kpi-grid--four">
        <KpiCard label="Permissão" value={ROLE_LABELS[profile.role]} detail="Nível de acesso ativo" icon={<ShieldCheck size={19} />} tone={profile.role === "admin" || profile.role === "director" ? "red" : "neutral"} />
      ...
      <Panel title="Escopo operacional" subtitle="Bases e siglas liberadas por RLS">
        <TableWrap>
          ...
          <tbody>
            {scopeRows.length ? scopeRows.map((row) => <tr key={`${row.type}-${row.value}`}><td>{row.type}</td><td><strong>{row.value}</strong></td></tr>) : <tr><td colSpan={2}>Nenhum escopo explícito cadastrado para este usuário.</td></tr>}
          </tbody>
        </TableWrap>
      </Panel>
    </div>
