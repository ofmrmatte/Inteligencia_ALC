...
import { ROLE_LABELS, canManageImports, type AuthProfile } from "@/lib/auth";
...
export function SettingsView({ profile }: { profile: AuthProfile }) {
  ...
  const rows = [
    ...
    { area: "Escopo", status: "RLS por base/sigla", owner: "Banco", detail: "Coordenadores e supervisores dependem do escopo cadastrado." },
    { area: "Auditoria", status: "Estrutura pronta", owner: "ADM", detail: "Eventos administrativos preparados na migração Supabase." },
  ];

  return (
    <div className="view-stack">
      <PageIntro description="Central de parâmetros administrativos da Inteligência ALC, alinhada ao controle de perfis e rastreabilidade dos lotes." chips={[`Perfil atual: ${ROLE_LABELS[profile.role]}`, canAdmin ? "Gestão completa" : "Consulta operacional"]} />
      <div className="kpi-grid kpi-grid--four">
        <KpiCard label="Perfil atual" value={ROLE_LABELS[profile.role]} detail="Permissão da sessão" icon={<ShieldCheck size={19} />} tone={canAdmin ? "red" : "neutral"} />
